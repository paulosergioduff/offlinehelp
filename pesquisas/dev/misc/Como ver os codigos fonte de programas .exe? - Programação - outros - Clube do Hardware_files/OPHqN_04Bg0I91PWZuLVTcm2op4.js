;if(CloudflareApps.matchPage(CloudflareApps.installs['qC0w1YKS0tj2'].URLPatterns)){(function(modules){var installedModules={};function __webpack_require__(moduleId){if(installedModules[moduleId]){return installedModules[moduleId].exports;}
...
var hash=void 0
var options=CloudflareApps.installs['qC0w1YKS0tj2'].options
var product=CloudflareApps.installs['qC0w1YKS0tj2'].product
...
if("qC0w1YKS0tj2"==='preview'&&node.nodeName==='IFRAME'&&node.src.indexOf('https://embedded.cloudflareapps.com')!==-1){return}
...
if("qC0w1YKS0tj2"==='preview'){messageIndex=previewMessageIndex}else{messageIndex=Math.floor(Math.random()*options.messages.length)}
...
if("qC0w1YKS0tj2"!=='preview'&&(!shouldShow||hasSeenHash())){hideWelcomeBar()
return}
updateElementStyle()
element.innerHTML=''
...
window.CloudflareApps.installs['qC0w1YKS0tj2'].scope={updateOptions:function updateOptions(nextOptions){if(nextOptions.messages.length!==options.messages.length){previewMessageIndex=nextOptions.messages.length-1}else{for(var i=0;i<nextOptions.messages.length;i++){var oldEntry=options.messages[i]
